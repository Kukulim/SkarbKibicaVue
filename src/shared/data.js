...
const getPlayers = async function(teamId, teamSquadId){
  try {
    const response = await axios.get(`/api/teams/${teamId}/TeamSquads/${teamSquadId}/players`);
...
const getPlayer = async function(teamId, teamSquadId, playerId){
  try {
    const response = await axios.get(`/api/teams/${teamId}/TeamSquads/${teamSquadId}/players/${playerId}`);
...
const createPlayer = async function(teamId, teamSquadId, player){
  try{
    await axios.post(`/api/teams/${teamId}/TeamSquads/${teamSquadId}/players/`, player, {headers: {"Content-Type": "application/json"}});
  } catch (error) {
    console.error(error);
    return null;
}};
...
const updatePlayer = async function(teamId, teamSquadId,playerId, player){
  try{
    await axios.put(`/api/teams/${teamId}/TeamSquads/${teamSquadId}/players/${playerId}`, player, {headers: {"Content-Type": "application/json"}});
  } catch (error) {
    console.error(error);
    return null;
}};
...
const deletePlayer = async function(teamId, teamSquadId, id){
  try{
    await axios.delete(`/api/teams/${teamId}/TeamSquads/${teamSquadId}/players/${id}`);
  } catch (error) {
    console.error(error);
    return null;
}};
...
const getTeamSquad = async function(teamId, teamSquadId){
  try {
    const response = await axios.get(`/api/teams/${teamId}/TeamSquads/${teamSquadId}`);
    const team = response.data;
    if (!team) return console.log("bład pusta druzyna");
    return team;
  }
